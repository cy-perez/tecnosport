...
| Transferencia internacional de datos: régimen propio, con lista de países de nivel adecuado | Ley 1581 de 2012 y circular de la SIC sobre nivel adecuado de protección | ~~**México aparece en la lista de países con nivel adecuado** de la SIC. Skydropx es mexicana; eso no cierra el análisis, lo simplifica.~~ **La premisa era falsa y se corrigió el 14 de septiembre de 2026:** con quien se contrata en Colombia es **SKYDROPX S.A.S., sociedad colombiana** (§4), así que compartirle los datos no es una transferencia internacional. Lo que su propio aviso de privacidad sí contempla es que ella transmita o transfiera datos, nacional o internacionalmente; eso lo declara y responde ella. Queda abierto **dónde procesa los datos la plataforma**, y ya no depende de saber cuál es el host: eso se cerró el 23 de septiembre —`api-pro.skydropx.com` en producción, `docs/13` §6.3— y **un nombre de dominio no dice dónde corre**. Depende del contrato y de lo que ella declare; si resulta que se procesan fuera, el numeral 9 de la política vuelve a cambiar |
...
| IVA sobre el flete cobrado | **Decidido el 18 de septiembre de 2026 (`adr/0040`): el flete cobrado no se grava.** Es una **decisión de negocio, sin concepto de contador**, tomada sabiendo que la lectura de la norma apunta al otro lado; el ADR escribe qué la reabre y a qué se volvería —sumarle el 19% al valor cotizado, nunca absorberlo—. El análisis que la precede, que sigue en pie: se había cerrado el 14 de septiembre creyendo que el valor de la cotización ya traía IVA; al verificarlo resultó que son **dos preguntas**. El servicio de transporte de carga comprado suelto está excluido —eso es lo que se encuentra al buscar y es cierto—, pero el flete que el vendedor le recobra al comprador dentro de una venta gravada integra la **base gravable** por el **art. 447 del Estatuto Tributario** ("acarreos… aunque se facturen o convengan por separado y aunque, considerados independientemente, no se encuentren sometidos a imposición"), y el **Concepto DIAN 4945 de 2025** lo confirma para el transporte que contrata el vendedor, incluso subcontratado. Eximiría solo que el comprador contratara el transporte por su cuenta, que no es lo que hace este sitio. **Consecuencia:** se cobra el `rate.total` sin sumarle el 19%, así que si el impuesto aplicara saldría del margen en cada pedido a domicilio —entre 1.138 y 1.880 con las tarifas que la cuenta cotiza hoy—, y **ese riesgo se asume a sabiendas**. Ver `adr/0040` y `docs/02-modelo-datos.md`. ~~El valor de la cotización ya viene con IVA y se cobra tal cual.~~ ~~El desglose del checkout y de la pantalla de estado dice que todos los valores incluyen IVA, **el del envío también**.~~ **Esa frase también caducó, y el mismo 14 de septiembre** (`96a7c69`): era falsa sobre la cifra que tenía al lado, así que el texto se estrechó a "Los precios de los productos incluyen IVA" y calla sobre el flete. Se queda así — callar es cierto en los tres escenarios, afirmar lo es solo en uno, y un sitio público no es donde se declara una posición tributaria que ningún contador firmó | Decidido por el negocio |
...
| ~~Peso y dimensiones del catálogo~~ | **No era un dato que faltara, era un procedimiento.** El `TODO` de `SembradorCatalogo` hablaba del catálogo sembrado, que es ficción declarada; el catálogo real entra por el panel. Lo que faltaba por decidir es **quién mide y con qué** al cargar producto real, y así quedó reescrito el `TODO`. **Matizado el 19 de septiembre de 2026 (`adr/0046`):** el panel ya no exige las cuatro medidas — una variante sin medir se publica y se vende solo con recogida en el punto, así que medir pasó de requisito de carga a mejora incremental. Lo que no cambia es la promesa de envío: un producto sin medir no se despacha a domicilio y el checkout lo dice nombrándolo | Decidido |
...
- [ ] **Un contador revisó `adr/0040`** — el IVA del flete cobrado. No bloquea
      ...
      lectura de la norma, a sabiendas y por decisión de negocio, y quien lo herede
      tiene que saberlo antes de que haya pedidos reales que arreglar hacia atrás.
