...
| Créditos del sandbox | ✅ **Repuestos. 102.238 COP medidos el 22 de septiembre de 2026** | Historia: Skydropx depositó 49.000 a mano el 15 (`transaction_source: Skydropx`, etiqueta `USO_INTERNO`); nunca funcionó la vía de autoservicio. Las emisiones de prueba lo bajaron a 388, por debajo de la tarifa más barata con recolección por API (5.991), y eso **bloqueó la recolección** (§6.7) durante una semana. Con 102.238 deja de bloquearla. El saldo se consulta con `GET /api/v1/finance/credits`, que es de lectura y no gasta. |
