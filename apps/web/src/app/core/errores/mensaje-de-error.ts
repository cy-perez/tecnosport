 ...
 *
 * Existe porque los paneles de administración hacían lo mismo: `catch { error.set(translate('…
 ...
 *
 * Lo usan cinco. Los de productos, variantes y edición siguen con el genérico, y ahí duele:
 * `SKU_YA_EN_USO` es información accionable que se ve como "no se pudo completar la acción".
 *
...
export function mensajeDeError(
  ...
  // El scope sale de la clave genérica que pasa la pantalla —`admin.retractos.error` da `admin`—
  // en vez de estar cableado: esto vive en `core/` y no tiene por qué saber que hoy solo lo llama el
  // panel de administración.
  const scope = claveGenerica.split('.')[0];
  const clave = `${scope}.errores.${codigo.toLowerCase()}`;
