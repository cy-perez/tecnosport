...
import { EjeAtributo } from '../../features/catalogo/domain/seleccion-variante';
...
describe('TsSelectorVariante', () => {
  ...
  it('marca la opción activa con aria-pressed', async () => {
    ...
    expect(screen.getByRole('button', { name: 'Azul marino' }).getAttribute('aria-pressed')).toBe('true');
    expect(screen.getByRole('button', { name: 'Negro' }).getAttribute('aria-pressed')).toBe('false');
  });
});
